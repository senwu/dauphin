# ACK: This code is originaly from:
#   https://github.com/hysts/pytorch_shake_shake
import torch
from torch import nn as nn
from torch.nn import functional as F

from dauphin.image.models.shake_shake_function import get_alpha_beta, shake_function
...
class ShakeShake(nn.Module):
    def __init__(
        ...
        self.shake_forward = shake_forward
        self.shake_backward = shake_backward
        self.shake_image = shake_image
        self.has_fc = has_fc

        self.shake_config = (self.shake_forward, self.shake_backward, self.shake_image)

        block = BasicBlock
        n_blocks_per_stage = (self.depth - 2) // 6
        assert n_blocks_per_stage * 6 + 2 == self.depth
        ...
        self.bn = nn.BatchNorm2d(self.base_channels)
    ...
    def __repr__(self):
        return (
            f"{type(self).__name__}-{self.depth} {self.base_channels} "
            f"{self.shake_forward} {self.shake_backward} {self.shake_image}"
        )
